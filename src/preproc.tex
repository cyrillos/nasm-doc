...
\section{\textindexlc{Standard Macros}}
\label{ch:stdmac}
...
\subsection{\textindexlc{NASM Version} Macros}
\label{sec:stdmacver}
...
\subsection{\codeindex{\_\_NASM\_VERSION\_ID\_\_}: \textindexlc{NASM Version ID}}
...
\subsection{\codeindex{\_\_NASM\_VER\_\_}: \textindexlc{NASM Version string}}
...
\subsection{\codeindex{\_\_FILE\_\_} and \codeindex{\_\_LINE\_\_}: File Name and Line Number}
\label{sec:fileline}
...
\subsection{\codeindex{\_\_BITS\_\_}: Current BITS Mode}
\label{sec:bitsm}
...
\subsection{\codeindex{\_\_OUTPUT\_FORMAT\_\_}: Current Output Format}
\label{sec:ofmtm}

The \code{\_\_OUTPUT\_FORMAT\_\_} standard macro holds the current output format,
as given by the \code{-f} option or NASM's default. Type \code{nasm -hf} for a
list.
...
\subsection{Assembly Date and Time Macros}
\label{sec:datetime}
...
\subsection{\indexcode{\_\_USE\_*\_\_}\code{\_\_USE\_}\emph{package}\code{\_\_}: Package Include Test}
\label{sec:usedef}

When a standard macro package (see \fullref{sec:macropkg}) is included with the
\code{\%use} directive (see \fullref{sec:use}), a single-line macro of
...
For example, if the \code{altreg} package is included (see \fullref{sec:pkgaltreg}),
then the macro \code{\_\_USE\_ALTREG\_\_} is defined.

\subsection{\codeindex{\_\_PASS\_\_}: Assembly Pass}
\label{sec:usedef}
...
\code{-M} or \code{-MG} option, see \fullref{sec:optM}) it is set to \code{0}.
...
\subsection{\codeindex{STRUC} and \codeindex{ENDSTRUC}: \textindexlc{Declaring Structure} Data Types}
\label{sec:struc}
...
\subsection{\codeindex{ISTRUC}, \codeindex{AT} and \codeindex{IEND}: Declaring}
\label{sec:istruc}
...
\subsection{\codeindex{ALIGN} and \codeindex{ALIGNB}: Data Alignment}
\label{sec:align}
...
See \fullref{sec:sectalign} for details.

See also the \code{smartalign} standard macro package, \fullref{sec:pkgsmartalign}.

\subsection{\codeindex{SECTALIGN}: Section Alignment}
\label{sec:sectalign}
...
Note that \code{ALIGN} (see \fullref{sec:align}) calls the \code{SECTALIGN}
