...
\subsection{\textindexlc{Thread Local Storage in Mach-O}\index{TLS}:
...
\subsection{\code{macho} specfic directive \codeindex{subsections\_via\_symbols}}
...
\subsection{\code{macho} specfic directive \codeindex{no\_dead\_strip}}
\label{subsec:macho-ssvs}
...
\section{\codeindex{elf32}, \codeindex{elf64}, \codeindex{elfx32}:
...
\subsection{ELF specific directive \codeindex{osabi}}
...
\subsection{\code{elf} extensions to the \code{SECTION} Directive}
...
\subsection{\textindexlc{Position-Independent Code}: \code{elf}
...
shared libraries entirely in NASM is given in \fullref{sec:picdll}.

\subsection{\textindexlc{Thread Local Storage in ELF}:
...
\subsection{\code{elf} Extensions to the \code{GLOBAL} Directive}
...
\fullref{sec:picglobal}.

\subsection{\code{elf} Extensions to the \code{COMMON} Directive}
...
\subsection{16-bit code and ELF}
\label{subsec:elf16}
...
\section{\codeindex{aout}: Linux \code{a.out} Object Files}
\label{sec:aoutfmt}
...
\fullref{sec:elffmt}.) These differ from other \code{a.out} object
...
\section{\codeindex{aoutb}: \textindex{NetBSD}/\textindex{FreeBSD}/\textindex{OpenBSD}
...
code relocation types. See \fullref{subsec:elfwrt} for full documentation
of this feature.

\code{aoutb} also supports the same extensions to the \code{GLOBAL}
directive as \code{elf} does: see \fullref{subsec:elfglob} for
documentation of this.

\section{\code{as86}: \textindex{Minix}/Linux \codeindex{as86} Object Files}
\label{sec:as86fmt}
\index{linux!as86}
...
\section{\index{RDOFF}\codeindex{rdf}: \textindexlc{Relocatable Dynamic
Object File Format}}
\label{sec:rdffmt}
...
\subsection{Requiring a Library: The \codeindex{LIBRARY} Directive}
\label{subsec:rdflib}
...
\subsection{Specifying a Module Name: The \codeindex{MODULE} Directive}
\label{subsec:rdfmod}
...
\subsection{\code{rdf} Extensions to the \code{GLOBAL} Directive}
\label{subsec:rdfglob}
...
\subsection{\code{rdf} Extensions to the \code{EXTERN} Directive}
\label{subsec:rdfimpt}
...
\section{\codeindex{dbg}: Debugging Format}
\label{sec:dbgfmt}
