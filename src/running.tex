\chapter{Running NASM}

\section{NASM \htextindex{Command-Line} Syntax}
...
\subsection{The \hcodeindex{-o} Option: Specifying the \htextindex{Output File Name}}
\label{subsec:opt-o}
...
\subsection{The \hcodeindex{-f} Option: Specifying the \htextindex{Output File Format}}
\label{subsec:opt-f}
...
\subsection{The \hcodeindex{-l} Option: Generating a \htextindex{Listing File}}
\label{subsec:opt-l}
...
\subsection{The \hcodeindex{-M} Option: Generate \htextindex{Makefile Dependencies}}
...
\subsection{The \hcodeindex{-MG} Option: Generate \htextindex{Makefile Dependencies}}
...
\subsection{The \hcodeindex{-MF} Option: Set Makefile Dependency File}
...
\subsection{The \hcodeindex{-MD} Option: Assemble and Generate Dependencies}
...
\subsection{The \hcodeindex{-MT} Option: Dependency Target Name}
...
\subsection{The \hcodeindex{-MQ} Option: Dependency Target Name (Quoted)}
...
\subsection{The \hcodeindex{-MP} Option: Emit phony targets}
...
\subsection{The \hcodeindex{-F} Option: Selecting a \htextindex{Debug Information Format}}
...
see \fullref{subsec:opt-g}, to enable output. Versions 2.03.01 and later
...
\subsection{The \hcodeindex{-g} Option: Enabling \htextindex{Debug Information}}
\label{subsec:opt-g}

This option can be used to generate debugging information in the specified
format. See \fullref{subsec:opt-F-cap}. Using \code{-g} without \code{-F}
...
\subsection{The \hcodeindex{-X} Option: Selecting an \htextindex{Error Reporting Format}}
...
\subsection{The \hcodeindex{-Z} Option: Send Errors to a File}
...
\subsection{The \hcodeindex{-s} Option: Send Errors to \hcodeindex{stdout}}
\label{subsec:opt-s}

The \code{-s} option redirects \htextindex{error messages} to
...
\subsection{The \hcodeindex{-i}\indexcode{-I} Option: Include File Search Directories}
\label{subsec:opt-i}
...
\subsection{The \hcodeindex{-p}\indexcode{-P} Option: \index{pre-including files}Pre-Include a File}
\label{subsec:opt-p}

\index{\%include}NASM allows you to specify files to be \emph{pre-included} into
...
\subsection{The \hcodeindex{-d}\indexcode{-D} Option: \index{pre-defining macros}Pre-Define a Macro}
\label{subsec:opt-d}

\index{\%define}Just as the \code{-p} option gives an alternative to placing
...
\subsection{The \hcodeindex{-u}\indexcode{-U} Option: \index{undefining macros}Undefine a Macro}
\label{subsec:opt-u}

\index{\%undef}The \code{-u} option undefines a macro that would otherwise
...
\subsection{The \hcodeindex{-E}\indexcode{-e} Option: Preprocess Only}
...
\subsection{The \hcodeindex{-a} Option: Don't Preprocess At All}
\label{subsec:opt-a}
...
\subsection{The \hcodeindex{-O} Option: Specifying \htextindex{Multipass Optimization}}
...
\subsection{The \hcodeindex{-t} Option: Enable TASM Compatibility Mode}
\label{subsec:opt-t}
...
\subsection{The \hcodeindex{-w} and \hcodeindex{-W} Options: Enable or Disable Assembly \htextindex{Warnings}}
\label{subsec:opt-w}
...
\subsection{The \hcodeindex{-v} Option: Display \htextindex{Version} Info}
\label{subsec:opt-v}
...
\subsection{The \hcodeindex{-y} Option: Display Available Debug Info Formats}
\label{subsec:opt-y}
...
\subsection{The \hcodeindex{--prefix} and \hcodeindex{--postfix} Options}
...
\subsection{The \hcodeindex{NASMENV} \textindex{Environment} Variable}
...
However, that means that the value \code{-dNAME="my name"} won't do
...
nonsensical words \code{-dNAME="my} and \code{name"}.
...
variable to the value \code{!-s!-ic:\\nasmlib\\} is equivalent to
setting it to \code{-s -ic:\\nasmlib\\}, but \code{!-dNAME=\"my name\"}
will work.

This environment variable was previously called \code{NASM}. This was
changed with version 0.98.31.

\section{\textindex{Quick Start} for \textindex{MASM} Users}
\label{subsec:qstart}
...
\subsection{NASM Is \index{case sensitivity}Case-Sensitive}
\label{subsec:qscs}
...
\subsection{NASM Requires \htextindex{Square Brackets} For \htextindex{Memory References}}
...
\subsection{NASM Doesn't Store \htextindex{Variable Types}}
...
\subsection{NASM Doesn't \hcodeindex{ASSUME}}
...
\subsection{NASM Doesn't Support \htextindex{Memory Models}}
...
\subsection{\htextindex{Floating-Point} Differences}
\label{subsec:qsfpu}
