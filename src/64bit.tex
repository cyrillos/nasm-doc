...
\code{REL} keyword (\fullref{sec:effaddr}). On most 64-bit platforms,
it is probably desirable to make that the default, using the directive
\code{DEFAULT REL} (\fullref{sec:default}).
...
the Standard C header \code{<inttypes.h>}.
...
\fullref{sec:pkg-altreg}) can be used for this purpose.
